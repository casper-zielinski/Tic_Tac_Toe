...
public class StartController implements Initializable {
    ...
    /**
     * Exit the application when the exit button is clicked. This method is
    ...
    /**
     * Start the game when the start button is clicked. This method is called
     * when the start button is clicked. A dialog is shown to select the game
     * mode (Player vs Player or Player vs Computer). The selected game mode is
     * then used to load the appropriate game scene. The Start Scene is closed
     * after the selection is made. If the user cancels the dialog, a message is
     * printed to the console and the application remains on the start screen
    ...
    void Start_game(ActionEvent event) {
        ...
        /**
         * Initialize the dialog for game selection. The dialog allows the user
         * to choose between Player vs Player and Player vs Computer modes.
         */
        ...
        if (result.isPresent() && result.get() == playerVsPlayer) {
            ...
            try {
                setPlayerVsXScene(false);
                stage = (Stage) btn_start.getScene().getWindow(); // Get the current stage
                stage.close(); // Close the current stage
            } catch (IOException e) {
                e.printStackTrace();
            } catch (NullPointerException e) { // You need to handle this exception to avoid NullPointerException and many others
                // Scene closed successfully
            }
        } else if (result.isPresent() && result.get() == playerVsComputer) {
            // Handle Player vs Computer selection
            try {
                setPlayerVsXScene(true);
                stage = (Stage) btn_start.getScene().getWindow(); // Get the current stage
                stage.close(); // Close the current stage
            } catch (IOException e) {
                e.printStackTrace();
    ...
    /**
     * Set the scene for Player vs X (either Player vs Player or Player vs
     * Computer). This method loads the TicTacToe.fxml file and sets the scene
     * for the game.
     *
     * @throws IOException if the FXML file cannot be loaded
     */
    private void setPlayerVsXScene(boolean playerVsComputer) throws IOException {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/TicTacToe.fxml"));
        root = loader.load();
        TicTacToeController controller = loader.getController();
        controller.setGameMode(playerVsComputer);
        stage = (Stage) btn_start.getScene().getWindow(); // Get the current stage
    ...
    /**
     * Show the rules of the game when the rules button is clicked. This method
     * is called when the rules button is clicked. It changes the style of the
     * buttons to indicate that the rules button is active.
     *
     * @param event
     */
    @FXML
